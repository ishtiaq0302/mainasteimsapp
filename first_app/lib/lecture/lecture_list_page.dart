...
class LectureListPage extends StatefulWidget {
  ...
  const LectureListPage({super.key, required this.userData});
...
class _LectureListPageState extends State<LectureListPage> {
  ...
  Future<void> _fetchLectures(String classesID) async {
    setState(() => _isLoading = true);
    String base = AppConfig.baseUrl;
    if (!base.endsWith('/')) base += '/';
    try {
      final response = await http.post(
        Uri.parse('${base}api/lecture'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
      ...
      if (!mounted) return;
      setState(() {
        _lectures    = result['data'] ?? [];
        _currentPage = 0;
        _filterLectures();
        _isLoading = false;
      });
    } catch (e) {
      dev.log('Fetch lectures error: $e');
      if (!mounted) return;
  ...
  void _filterLectures() {
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        _filteredLectures = _lectures.where((item) {
  ...
  Future<void> _deleteLecture(int id) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('${base}api/lecture_delete'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'lectureID': id,
          'adminID': widget.userData['adminID'] ?? 1,
      ...
      dev.log('Delete lecture error: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    final totalPages = (_filteredLectures.length / _rowsPerPage).ceil();
    final startIdx   = _currentPage * _rowsPerPage;
    final endIdx     = (startIdx + _rowsPerPage).clamp(0, _filteredLectures.length);
    final pageItems  = _filteredLectures.sublist(startIdx, endIdx);
          ...
          Expanded(
            child: _isLoading
                ...
                : _selectedCampus == null
                    ...
                    : _selectedClass == null
                        ? const Center(child: Text('Please select a class to view lectures'))
                                                    ...
                                                    PopupMenuButton<String>(
                                                      ...
                                                      onSelected: (value) {
                                                        if (value == 'download') {
                                                          ...
                                                          _confirmDelete(lectureID, title);
                                                        }
                                                      },
                                                      itemBuilder: (_) => [
                                                        if (hasFile)
                                    ...
                                    Padding(
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          Text(
                                            '${startIdx + 1}–$endIdx of ${_filteredLectures.length}',
                                            style: TextStyle(
