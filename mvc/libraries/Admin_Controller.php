...
*/
	...
	public function __construct () {
		...
		if(!empty($this->session->userdata('usertypeID')))
		{
		...
		if($this->session->userdata('usertypeID') == 1 && $this->session->userdata('adminLogin') == 1) {

			if(isset($userdata['adminLogin']) && !isset($userdata['get_permission'])) {
...

    {
        ...
        $campusID = $this->permission_m->get_campus_id();
    ...
    public function getCampusesDetails()
    ...
    public function getCampusReport()
	...
	private function alertMessageChecker() {
		...
		if ($this->data['messageConversations']) {
			...
			if(!empty($this->data['messageConversations'])) {
	...
	public function classcall() {
		...
		$campusID = $this->input->post('id'); 
			...
			$allclass = $this->classes_m->general_get_order_by_classes(array('campusID' => $campusID));

			echo "<option value='0'>", $this->lang->line("student_select_class"),"</option>";
	...
	public function guardiancall() {
		...
		$campusID = $this->input->post('id'); 
			...
			$allparents = $this->parents_m->get_order_by_parents(array('campusID' => $campusID));
			...
			foreach ($allparents as $value) {
				 ...
				 $parentsemail = '';
                                        ...
                                        if($value->email) {
                                            ...
                                            $parentsemail = " (" . $value->email ." )";

                                        }

                                        

				echo "<option value=\"$value->parentsID\">",$value->name.$parentsemail,"</option>";
