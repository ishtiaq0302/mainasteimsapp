...
class Clickatell
{
    ...
    public function __construct()
    {
        ...
        if($this->ci->session->userdata('accountCampusID')==0)
        {
            $camID = $this->ci->session->userdata('campus_id');
        }else{
            $camID = $this->ci->session->userdata('accountCampusID');            
        }
        $siteinfo = $this->ci->site_m->get_site_($camID);
