...
*/
	function __construct() {
		...
		$this->lang->load('lecture', $language);	
	}
	...
	public function index() {
		...
		if((int)$id) {
			...
			if(isset($fetchClasses[$id])) {
				...
				$this->data['syllabuss'] = $this->lecture_m->get_order_by_lecture(array('schoolyearID' => $schoolyearID, 'classesID' => $id, 'campusID' => $campusID));
				// echo "<pre>";
				// print_r($this->data['syllabuss']);
				// echo "</pre>";
				// exit();
				$this->data["subview"] = "lecture/index";
		...
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) {
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) { 
					...
					redirect(base_url("lecture/index/".$this->input->post("classesID").'/'.$this->input->post("campusID")));
				}
			} else {
				$this->data["subview"] = "lecture/add";
				$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		...
		if((int)$id && (int)$url) {
			if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1))  {
				$this->data['classes'] = $this->classes_m->get_classes();
				$this->data['syllabus'] = $this->lecture_m->get_single_lecture(array('lectureID' => $id, 'classesID' => $url, 'schoolyearID' => $schoolyearID));
				$this->data['campusID']=$this->data['syllabus']->campusID;
				$this->data['classesID']=$this->data['syllabus']->classesID;
				$fetchClasses = pluck($this->data['classes'], 'classesID', 'classesID');
				if(isset($fetchClasses[$url])) {
					if(!empty($this->data['syllabus'])) {
						if($_POST) {
							$rules = $this->rules();
							$this->form_validation->set_rules($rules);
							if ($this->form_validation->run() == FALSE) {
								$this->data["subview"] = "lecture/edit";
								$this->load->view('_layout_main', $this->data);			
							} else {
								$array = array(
									"campusID" => $this->input->post("campusID"),
								...
								$array['originalfile'] = $this->upload_data['file']['original_file_name'];
								$array['file'] = $this->upload_data['file']['file_name'];
								$this->lecture_m->update_lecture($array, $id);	
								$this->session->set_flashdata('success', $this->lang->line('menu_success'));
								redirect(base_url("lecture/index/$url".'/'.$this->input->post("campusID")));
							}
						} else {
							$this->data["subview"] = "lecture/edit";
							$this->load->view('_layout_main', $this->data);
	...
	public function lecture_list() {
		...
		$campusID = $this->input->post('campusID');
		if((int)$classID) {
			$string = base_url("lecture/index/$classID/$campusID");
			echo $string;
