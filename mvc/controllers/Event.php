...
*/
	...
	public function index() {
		...
		$this->data['events'] = $this->event_m->get_order_by_event(array('schoolyearID' => $schoolyearID,'campusID' => $campusID));
		...
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) {
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					redirect(base_url("event/index/".$array["campusID"]));
				}
			} else {
				$this->data["subview"] = "event/add";
				$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) {
			...
			if((int)$id) {
				...
				$this->data['campusID']=$this->data['event']->campusID;
	...
	public function print_preview() {
		if(permissionChecker('event_view')) {
			...
			if((int)$id) {
				$schoolyearID = $this->session->userdata("defaultschoolyearID");
				$this->data['event'] = $this->event_m->get_single_event(array('eventID' => $id, 'schoolyearID' => $schoolyearID));
				$this->data["siteinfos"] = $this->site_m->get_site_($this->data["event"]->campusID);
				if(!empty($this->data['event'])) {
