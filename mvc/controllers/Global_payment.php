    ...
    public function index() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if(!empty($this->data['single_student'])) {
    ...
    public function paymentSend() {
        ...
        if($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $globalpayment['campusID']          = $campusID;
                ...
                if($studentID) {
                    $student = $this->studentrelation_m->get_single_student(array('srcampusID' => $campusID,'srstudentID' => $studentID, 'srschoolyearID' => $schoolyearID));
                    if(!empty($student)) {
                ...
                if($globalLastID) { 
                    ...
                    for($i = 0; $i <= $j; $i++) {
                        $expPaidField = explode('-', $paids[$i]['paidFieldID']);
                        if(isset($expPaidField[1]) && isset($invoices[$i]) && !empty($invoices[$i]) && ($expPaidField[1] == $invoices[$i]->invoiceID)) {
                            $invoiceID = $expPaidField[1];
                            $invoice = $invoices[$i]; 
