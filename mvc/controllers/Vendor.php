...
class Vendor extends Admin_Controller {
    ...
    */
    ...
    public function index() {
        ...
        $this->data['vendors'] = $this->vendor_m->get_order_by_vendor(array('campusID'=>$campusID));
    ...
    public function add() {
        ...
        if($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                redirect(base_url("vendor/index/".$array['campusID']));
            }
        } else {
            $this->data["subview"] = "/vendor/add";
            $this->load->view('_layout_main', $this->data);
    ...
    public function edit() {
        ...
        if((int)$id) {
            ...
            $this->data['campusID']=$this->data['vendor']->campusID;
            if($this->data['vendor']) {
                if($_POST) {
                    $rules = $this->rules();
                    $this->form_validation->set_rules($rules);
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        $this->session->set_flashdata('success', $this->lang->line('menu_success'));
                        redirect(base_url("vendor/index/".$this->data['campusID']));
                    }
                } else {
                    $this->data["subview"] = "/vendor/edit";
                    $this->load->view('_layout_main', $this->data);
