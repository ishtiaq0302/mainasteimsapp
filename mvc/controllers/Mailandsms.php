...
*/
	...
	public function index() {
		...
		$this->data['mailandsmss'] = $this->mailandsms_m->get_mailandsms_with_usertypeID($campusID);
	...
	public function add($campusID=0) {
		...
		$camID= $campusID;
		$getCampus = count($this->getCampusDetails());
		...
		if($camID==0)
		{
			if($getCampus==1)
			{
				...
				redirect(base_url('mailandsms/add/'.$campusID));
			}
		}
						...
						if($studentID == 'select') {
							...
							if($class == 'select') {
								...
								if($schoolyear == 'select') {
									...
									if(!empty($multiSchoolYearStudents)) {
										$countusers = '';
									...
									if(!empty($singleSchoolYearStudents)) {
										$countusers = '';
						...
						if($systemadminID == 'select') {
							...
							if(!empty($multisystemadmins)) {
								...
								foreach ($multisystemadmins as $key => $multisystemadmin) {
									...
									/* START Check If phone number is empty or not correct then remove that record from send SMS and continue Other SMS Process */
	 								$phoneNumber = $multisystemadmin->phone;
									$rest = substr($phoneNumber, 0,2);
									if($rest=='92'){
	 									$str_to_replace = '0'; 
										$phoneNumber = $str_to_replace . substr($phoneNumber, 2); 
									}   
	...
	private function userConfigEmail($message, $user, $usertypeID, $schoolyearID = 1) {
		if($user && $usertypeID) {
			...
			if($user->email) {
				...
				$emailsetting = $this->emailsetting_m->get_emailsettings();
	...
	public function allusers() {
		if($this->input->post('usertypeID') == 'select') {
			...
			if($usertypeID == 1) {
					$systemadmins = $this->systemadmin_m->get_order_by_systemadmin(array('campusID' => $campusID));
				if(!empty($systemadmins)) {
					...
					$teachers = $this->teacher_m->general_get_order_by_teacher(array('campusID' => $campusID));
				if(!empty($teachers)) {
					echo "<option value='select'>".$this->lang->line('mailandsms_all_users')."</option>";
					...
					$classes = $this->classes_m->general_get_order_by_classes(array('campusID' => $campusID));
				if(!empty($classes)) {
				...
				if(!empty($parents)) {
					...
					$users = $this->user_m->get_order_by_user(array('usertypeID' => $usertypeID,'campusID' => $campusID));
				if(!empty($users)) {
					echo "<option value='select'>".$this->lang->line('mailandsms_all_users')."</option>";
		...
		if($this->input->post('sms_getway') == 'select') {
			...
			if(in_array($getway, $arrgetway)) {
				...
				elseif($getway == 'bulk') 
				{
					...
					/*if($this->sendpk->ping($to,$message) == TRUE) {*/ 
					if($this->bulk->ping($to,$message) == TRUE) { 
						return TRUE;
					} else {  
						if($this->session->userdata('bulk_msg')=='INVUP'){
							$this->form_validation->set_message("check_getway", 'Invalid Username or Password'); 
						}
						if($this->session->userdata('bulk_msg')=='INVUPA'){
							$this->form_validation->set_message("check_getway", 'Invalid Username Or Password Or API KEY'); 
						}
						if($this->session->userdata('bulk_msg')=='INVU'){
							$this->form_validation->set_message("check_getway", 'Please Type Username'); 
						}
						if($this->session->userdata('bulk_msg')=='INVCA'){
							$this->form_validation->set_message("check_getway", 'Check Your Account'); 
						}
						if($this->session->userdata('bulk_msg')=='PRMN'){
							$this->form_validation->set_message("check_getway", 'Please Type Receiver Mobile Number'); 
						}
	     				
	     				$this->session->unset_userdata('bulk_msg');
	     				return FALSE;
					} 
				}
	...
	private function allgetway_send_message($getway, $to, $message) {
		...
		elseif($getway == 'bulk') 
		{
			...
			if($to) {
				 	/*$this->sendpk->send_new($to,$message);*/
				 	$this->bulk->send_new($to,$message);

				 	if($this->session->userdata('mult_bulk_msg')=='UP')
				 	{
						$result['message'] = "Invalid Username or Password";
				 		$result['check'] = FALSE;
				 		$this->session->unset_userdata('mult_bulk_msg');
						return $result;
					}
					
					if($this->session->userdata('mult_bulk_msg')=='UPA')
					{
						$result['message'] = "Invalid Username Or Password Or API KEY";
				 		$result['check'] = FALSE;
				 		$this->session->unset_userdata('mult_bulk_msg');
						return $result;
					}
					
					if($this->session->userdata('mult_bulk_msg')=='U')
					{
						$result['message'] = "Please Type Username";
				 		$result['check'] = FALSE;
				 		$this->session->unset_userdata('mult_bulk_msg');
						return $result;
					}

					if($this->session->userdata('mult_bulk_msg')=='INVMN')
					{
						$result['message'] = "Invalid Number! Please Type Correct Mobile Number";
				 		$result['check'] = FALSE;
				 		$this->session->unset_userdata('mult_bulk_msg');
						return $result;
					}
					
					if($this->session->userdata('mult_bulk_msg')=='PA')
					{
						$result['message'] = "Checck Your Bulk Account";
				 		$result['check'] = FALSE;
				 		$this->session->unset_userdata('mult_bulk_msg');
						return $result;
					}
			...
			/*if($to) {
 			...
 			/*if($to) {
 				...
 				if($this->sendpk->send_new($to, $message) == TRUE)  {
					$result['check'] = TRUE;
					return $result;
				} else {
					$result['check'] = FALSE;
	...
	private function resultTableEmail($studentID, $classesID, $schoolyearID) {
		...
		if(!empty($markArray['separatedMarks'])) {
			if(!empty($markArray['exams'])) { 
						...
						if(!empty($markArray['separatedMarks'])) { 
							if(isset($markArray['separatedMarks'][$markArrayExam->examID])) {
								$string.= '<div style="border: 1px solid #ddd;margin:5px;display:inline-block">';
									...
									$totalMark=0;
									if(!empty($markArray['markpercentages'])) {
										$string.= '<table class="table-bordered">';
											...
											foreach ($markArray['separatedMarks'][$markArrayExam->examID] as $markArraySeparatedMarkKey => $markArraySeparatedMark) {
												if(!empty($markArraySeparatedMark)) {
														$string.= '<tr>';
															$string.= '<td style="border: 1px solid #ddd;padding: 8px;line-height: 1.428571429;vertical-align: top;">'; 
