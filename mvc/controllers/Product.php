...
class Product extends Admin_Controller {
    ...
    */
    ...
    public function index() {
        ...
        $this->data['products'] = $this->product_m->get_order_by_product(array('campusID'=>$campusID));
    ...
    public function add() {
       ...
       // view($this->data['productcategorys'],1);
        if($_POST) {
            $rules = $this->rules();
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                redirect(base_url("product/index/".$array['campusID']));
            }
        } else {
            $this->data["subview"] = "product/add";
            $this->load->view('_layout_main', $this->data);
    ...
    public function edit() {
        ...
        if((int)$id) {
            ...
            $this->data['campusID'] = $this->data['product']->campusID;
            $this->data['productcategoryID'] = $this->data['product']->productcategoryID;
            $this->data['productcategorys'] = $this->productcategory_m->get_productcategory();
    ...
    public function unique_productname() {
        $id = htmlentities(escapeString($this->uri->segment(3)));
        if((int)$id) {
            $product = $this->product_m->get_order_by_product(array("productname" => $this->input->post("productname"), "productID !=" => $id, "campusID" => $this->input->post("campusID")));
            if(!empty($product)) {
            ...
            $product = $this->product_m->get_order_by_product(array("productname" => $this->input->post("productname"), "campusID" => $this->input->post("campusID")));
            if(!empty($product)) {
                $this->form_validation->set_message("unique_productname", "The %s is already exists.");
                return FALSE;
            }
