...
*/
	...
	public function index() {
		...
		if((int)$id) {
			...
			if(isset($fetchClass[$id])) {
				$this->data['subjects'] = $this->subject_m->general_get_order_by_subject(array('classesID' => $id,'campusID' => $campusID));
	...
	public function add() {
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				if(!empty($this->input->post('teacherID'))) {
				...
				if(!empty($subjectteacherArray)) {
					$this->subjectteacher_m->insert_batch_subjectteacher($subjectteacherArray);
				}

				$this->session->set_flashdata('success', $this->lang->line('menu_success'));
				redirect(base_url("subject/index/".$this->input->post("classesID").'/'.$this->input->post("campusID")));
			}
		} else {
			$this->data["subview"] = "subject/add";
			$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		...
		if((int)$id && (int)$url) {
			...
			if(isset($fetchClass[$url])) {
				...
				$this->data['classesID']=$this->data['subject']->classesID;
				$this->data['campusID']=$this->data['subject']->campusID;
				...
				if(!empty($this->data['subject'])) {
					...
					if($_POST) {
						...
						if ($this->form_validation->run() == FALSE) {
							...
							$this->session->set_flashdata('success', $this->lang->line('menu_success'));
							redirect(base_url("subject/index/$url/".$this->input->post("campusID")));
						}
					} else {
						$this->data["subview"] = "subject/edit";
						$this->load->view('_layout_main', $this->data);
	...
	public function unique_subject_code() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$subject = $this->subject_m->general_get_order_by_subject(array("subject_code" => $this->input->post("subject_code"), "subjectID !=" => $id, "campusID" => $this->input->post("campusID")));
			if(!empty($subject)) {
			...
			$subject = $this->subject_m->general_get_order_by_subject(array("subject_code" => $this->input->post("subject_code"), "campusID" => $this->input->post("campusID")));
	...
	public function subject_list() {
		...
		$campusID = $this->input->post('campusID');
		if((int)$classID) {
			$string = base_url("subject/index/$classID/$campusID");
			echo $string;
		} else {
			redirect(base_url("subject/index"));
		}
