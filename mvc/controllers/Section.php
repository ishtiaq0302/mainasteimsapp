...
*/
	...
	public function index() {
		...
		if((int)$id) {
			...
			if(isset($fetchClass[$id])) {
				...
				$this->data['sections'] = $this->section_m->general_get_order_by_section(array('classesID' => $id,'campusID' => $campusID));
			} else {
				$this->data['teacher'] = [];
				$this->data['sections'] = [];
			}
	...
	public function add() {
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url("section/index/".$this->input->post('classesID').'/'.$this->input->post('campusID')));
			}
		} else {
			$this->data["subview"] = "section/add";
			$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		...
		if((int)$id && (int)$url) {
			...
			$this->data['campusID'] = $this->data['section']->campusID;
			$this->data['classesID'] = $this->data['section']->classesID;
			$this->data['teacherID'] = $this->data['section']->teacherID;
			$fetchClass = pluck($this->data['classes'], 'classesID', 'classesID');
				...
				if(!empty($this->data['section'])) {
					...
					if($_POST) {
						...
						if ($this->form_validation->run() == FALSE) {
							...
							$this->session->set_flashdata('success', $this->lang->line('menu_success'));
							redirect(base_url("section/index/$url".'/'.$array['campusID']));
						}
					} else {
						$this->data["subview"] = "section/edit";
						$this->load->view('_layout_main', $this->data);
	...
	public function delete() {
		...
		if((int)$id && (int)$url) {
			...
			if(!empty($section) && !empty($classes)) {
	...
	public function section_list() {
		...
		$campusID = $this->input->post('campusID');
		if((int)$classID) {
			$string = base_url("section/index/$classID/$campusID");
			echo $string;
		} else {
			redirect(base_url("section/index"));
		}
