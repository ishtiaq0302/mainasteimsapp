                            ...
                            if($_POST) {
                                if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
                                    ...
                                    if ($this->form_validation->run() == FALSE) {
                                        ...
                                        redirect(base_url("make_payment/add/$userID/$usertypeID/$campusID"));
                                    }
                                }
                            } else {
                                $this->data["subview"] = "make_payment/add";
    ...
    public function view() {
        if(permissionChecker('make_payment')) {
            ...
            if((int)$id) {
                ...
                if(!empty($this->data['make_payment'])) {
                    ...
                    if((int)$userID && (int) $usertypeID) {
                        ...
                        if(!empty($user)) {
                            ...
                            if(!empty($manageSalary)) {
                                ...
                                if(!empty($this->data['persent_salary_template'])) {
                                    if($this->data['make_payment']->salaryID == 1) {
                                        ...
                                        if(!empty($this->data['salary_template'])) {
                                            $this->db->order_by("salary_optionID", "asc");
    ...
    public function print_preview() {
        if(permissionChecker('make_payment')) {
            ...
            if((int)$id) {
                ...
                $schoolyearID = $this->session->userdata('defaultschoolyearID');
                $this->data['make_payment'] = $this->make_payment_m->get_single_make_payment(array('make_paymentID' => $id, 'schoolyearID' => $schoolyearID));
                $this->data["siteinfos"] = $this->site_m->get_site_($this->data["make_payment"]->campusID);
                if(!empty($this->data['make_payment'])) {
                    $userID = $this->data['make_payment']->userID;
                    $usertypeID = $this->data['make_payment']->usertypeID;
            ...
            if(permissionChecker('make_payment')) {
                ...
                if((int)$id) {
                    ...
                    if($this->data['make_payment']) {
                        ...
                        $this->session->set_flashdata('success', $this->lang->line('menu_success'));
                        redirect(base_url("make_payment/add/".$this->data['make_payment']->userID.'/'.$this->data['make_payment']->usertypeID.'/'.$campusID));
                    } else {
