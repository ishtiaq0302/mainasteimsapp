...
*/
	...
	public function index() {
		...
		if($usertypeID == 3) {
			$this->data['payments'] = $this->payment_m->get_payment_with_studentrelation_by_studentID($userID, $schoolyearID,$campusID);
			...
			$students = $this->studentrelation_m->get_order_by_student(array('parentID' => $userID, 'schoolyearID' => $schoolyearID, 'campusID' => $campusID));
			if(!empty($students)) {
				$studentArray = pluck($students, 'srstudentID');
				$this->data['payments'] = [];
				$this->data['payments'] = $this->payment_m->get_payment_with_studentrelation_by_studentID($studentArray, $schoolyearID,$campusID);
			...
			$this->data['payments'] = $this->payment_m->get_payment_with_studentrelation($schoolyearID,$campusID);
			$this->data["subview"] = "paymenthistory/index";
			$this->load->view('_layout_main', $this->data);
		}
	}
	...
	public function edit() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('defaultschoolyearID') == 5)) {
			...
			if((int)$id) {
				...
				$this->data['campusID']=$this->data['payment']->campusID;
