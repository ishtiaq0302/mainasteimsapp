...
class Asset extends Admin_Controller {
    ...
    */
    ...
    public function index() {
        ...
        $this->data['assets'] = $this->asset_m->get_asset_with_category_and_locations($campusID);
    ...
    public function add() {
        ...
        if($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                redirect(base_url("asset/index/".$array['campusID']));
            }
        } else {
            $this->data["subview"] = "asset/add";
            $this->load->view('_layout_main', $this->data);
    ...
    public function edit() {
        ...
        if((int)$id) {
            ...
            $this->data['campusID']=$this->data['asset']->campusID;
            $this->data['asset_locationID']=$this->data['asset']->asset_locationID;
    ...
    public function print_preview() {
        if(permissionChecker('asset_view')) {
            ...
            if((int)$assetID) {
                ...
                $this->data["siteinfos"] = $this->site_m->get_site_($this->data["asset"]->campusID);
                if(!empty($this->data['asset'])) {
    ...
    public function unique_serial() {
        $id = htmlentities(escapeString($this->uri->segment(3)));
        if((int)$id) {
            $asset = $this->asset_m->get_single_asset(array("serial" => $this->input->post("serial"), "assetID !=" => $id, "campusID" => $this->input->post("campusID")));
            if(!empty($asset)) {
            ...
            $asset = $this->asset_m->get_single_asset(array("serial" => $this->input->post("serial"), "campusID" => $this->input->post("campusID")));
