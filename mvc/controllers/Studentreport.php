...
*/
	...
	public function getStudentReport()
	{
		...
		if(permissionChecker('studentreport')) {
			if($_POST) {
				...
				if($campusID==0){
					$this->data["siteinfos"] = $this->site_m->get_site_(1);
				}else{
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if($reportfor == 'transport') {
						if($campusID>0){
							$transports = $this->tmember_m->get_order_by_tmember(array('transportID' => $transport,'campusID' => $campusID));
						}else{
						...
						if($campusID>0){
							$hostels = $this->hmember_m->get_order_by_hmember(array('hostelID' => $hostel,'campusID' => $campusID));
						}else{
						...
						$this->data['students'] = $this->studentrelation_m->general_get_order_by_student($queryArray,array('campusID'=>$campusID));
					}
	...
	private function getArray(&$queryArray, $post) {
		...
		if($campusID > 0) {
			$queryArray['srcampusID'] = $campusID;
		}
		...
		$this->data['campuse'] 	= pluck($this->campus_m->get_campus(), 'obj' , 'campusID');
	}
	...
	public function pdf() {
		if(permissionChecker('studentreport')) {
			...
			$reportfor      = htmlentities(escapeString($this->uri->segment(4)));
			$bloodID        = htmlentities(escapeString($this->uri->segment(5)));
			$country        = htmlentities(escapeString($this->uri->segment(6)));
			$transport      = htmlentities(escapeString($this->uri->segment(7)));
			$hostel         = htmlentities(escapeString($this->uri->segment(8))); 
			$gender         = htmlentities(escapeString($this->uri->segment(9))); 
			$birthdaydate   = htmlentities(escapeString($this->uri->segment(10)));
			$classesID      = htmlentities(escapeString($this->uri->segment(11)));
			$sectionID      = htmlentities(escapeString($this->uri->segment(12)));
	...
	private function uriChecker() {
		$totalUri = !empty($this->uri->segment_array());
