...
*/
	...
	public function index() {
		...
		$setcampusID = htmlentities(escapeString($this->uri->segment(3)));
		if($setcampusID==0){
			if($this->session->userdata('campus_id')==0)
			{
				$setcampusID = $this->session->userdata('accountCampusID');
				redirect(base_url('setting/index/'.$setcampusID));
			}else{
				$setcampusID = $this->session->userdata('campus_id');
				redirect(base_url('setting/index/'.$setcampusID));
			}
		}


		$this->data['setcampusID']=$setcampusID;

		$this->data['getCampus'] = $this->getCampusDetails();
		$usertype = $this->session->userdata("usertype");

		$this->data['setting'] = $this->setting_m->get_setting($setcampusID);
		//$this->data['unique_setting'] = $this->setting_m->get_unique_setting(1);
		$this->data['unique_setting'] = $this->setting_m->get_setting($setcampusID);
		if(!empty($this->data['settings']))
		{
			$this->data['setting'] = $this->data['setting'];
		}else{
			$this->data['setting'] =$this->data['unique_setting'];
		}

		//$this->data['settingarray'] = $this->setting_m->get_setting_array();
		$this->data['settingarray'] = $this->data['unique_setting'];

		$this->data['markpercentages'] = $this->markpercentage_m->get_markpercentages(array('campusID'=>$setcampusID));

		$this->data['schoolyears'] = $this->schoolyear_m->get_order_by_schoolyear(array('schooltype' => $this->data['unique_setting']->school_type));

		$this->data['idmanagers'] = $this->idmanager_m->get_order_by_idmanager(array('schooltype' => $this->data['unique_setting']->school_type));
...
*/
					...
					$array['zoom_api_key'] = $this->input->post('zoom_api_key');
					$array['zoom_api_secret'] = $this->input->post('zoom_api_secret');
					...
					$markperc = $this->markpercentage_m->get_markpercentage();					
					if(!empty($markperc))
					{
						$mark_array = count($markperc);
					}else{
						$mark_array = 0;						
					}
					//view($markperc,1);
					...
					$this->setting_m->updatesettings($array,$campusID,$markperc);
					//$this->setting_m->insertorupdate($array,$campusID);
                    ...
                    frontendData::get_backend($search = NULL,$campusID);
...
//die;
	...
	public function backendtheme() {
		...
		$campusID = htmlentities(escapeString($this->input->post('campusID')));
		$themeName = 'default';
		...
		$this->setting_m->update_settings($campusID, $themeName);
		echo $themeName;
