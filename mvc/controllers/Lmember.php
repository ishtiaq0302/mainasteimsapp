...
*/
		...
		if($this->session->userdata('usertypeID') == 3 && $myProfile) {
			...
			if((int)$id) {
				...
				if(isset($fetchClass[$id])) {
					$this->data['students'] = $this->studentrelation_m->get_order_by_student(array('srcampusID' => $campusID,'srclassesID' => $id, 'srschoolyearID' => $schoolyearID));
					...
					if(!empty($this->data['students'])) {
						...
						foreach ($sections as $key => $section) {
							$this->data['allsection'][$section->sectionID] = $this->studentrelation_m->get_order_by_student(array('srclassesID' => $id, "srsectionID" => $section->sectionID, 'srschoolyearID' => $schoolyearID, 'campusID' => $campusID));
						}
					} else {
						$this->data['students'] = [];
					}
	...
	public function add() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			$this->data['campusID'] = htmlentities(escapeString($this->uri->segment(5)));
				...
				if(!empty($student)) {
					...
					if($_POST) {
						...
						if ($this->form_validation->run() == FALSE) {
							...
							redirect(base_url("lmember/index/$url/".$this->data['campusID']));
						}
					} else {
						$this->data["subview"] = "lmember/add";
						$this->load->view('_layout_main', $this->data);
	...
	public function student_list() {
		...
		if((int)$classID) {
			$string = base_url("lmember/index/$classID/$campusID");
			echo $string;
		} else {
			redirect(base_url("lmember/index"));
		}
