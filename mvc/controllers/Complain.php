...
class Complain extends Admin_Controller {
    ...
    */
    ...
    public function index() {
        ...
        if($usertypeID == 1) {
            $this->data['complains'] = $this->complain_m->get_order_by_complain(array('schoolyearID' => $schoolyearID,'campusID' => $campusID));
            $this->data["subview"] = "complain/index";
            $this->load->view('_layout_main', $this->data);
        } else {
            $this->data['complains'] = $this->complain_m->get_order_by_complain(array('schoolyearID' => $schoolyearID,'campusID' => $campusID, 'create_userID' => $userID, 'create_usertypeID' => $usertypeID));
            $this->data["subview"] = "complain/index";
            $this->load->view('_layout_main', $this->data);
        }
    }
        ...
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
            ...
            if($_POST) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    redirect(base_url("complain/index/".$array['campusID']));
                }
            } else {
                $this->data["subview"] = "complain/add";
                $this->load->view('_layout_main', $this->data);
    ...
    public function edit() {
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
            ...
            if((int)$id) {
                ...
                $this->data['campusID']=$this->data['complain']->campusID;
    ...
    public function print_preview() {
        if(permissionChecker('complain_view')) {
            ...
            if((int)$id) {
                ...
                $this->data["siteinfos"] = $this->site_m->get_site_($this->data["complain"]->campusID);
                if(!empty($this->data['complain'])) {
                    $usertypeID = $this->data['complain']->usertypeID;
                    $userID     = $this->data['complain']->userID;
                    $this->data['createinfo'] = getObjectByUserTypeIDAndUserID($this->data['complain']->create_usertypeID, $this->data['complain']->create_userID, $schoolyearID);
    ...
    public function allStudent() {
        ...
        if( (int)$classesID) {
            $schoolyearID = $this->session->userdata('defaultschoolyearID');
            $students = $this->studentrelation_m->general_get_order_by_student(array('srclassesID' => $classesID, 'srschoolyearID' => $schoolyearID, 'srcampusID' => $campusID));
    ...
    public function allusers() {
        $loginuserID = $this->session->userdata('loginuserID');
        //$usertypeID = $this->session->userdata('usertypeID');
        $usertypeIDs = $this->input->post('usertypeID');
        $campusID = explode('/', $usertypeIDs, 2)[1];
        $usertypeID = strtok($usertypeIDs, '/');
        ...
        if($this->input->post('usertypeID') == '0') {
            ...
            if($usertypeID == 1) {
                $systemadmins = $this->systemadmin_m->get_order_by_systemadmin(array('campus_id' => $campusID));
                ...
                $classes = $this->classes_m->get_order_by_classes(array('campusID' => $campusID));
                if(!empty($classes)) {
