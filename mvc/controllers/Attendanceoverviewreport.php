...
class Attendanceoverviewreport extends Admin_Controller {
	...
	public function getUser() {
		...
		if((int)$usertype) {
			...
			if($usertype == 2) {
				...
				if($campusID>0)
				{
					$users = $this->user_m->get_order_by_user(array('campusID'=>$campusID));
				}else{
	...
	private function userQueryArray($posts) {
		$userQueryArray = [];
			if($posts['campusID'] > 0) {
				$userQueryArray['campusID'] = $posts['campusID'];
			}
		if($posts['usertype'] == '1') {
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
	...
	private function queryArray($posts) {
		...
		$queryArray['schoolyearID'] = $schoolyearID; 				
			if($posts['campusID'] > 0) {
				$queryArray['campusID'] = $posts['campusID'];
			}
		if($posts['monthID'] !='') {
	...
	public function pdf() {
		if(permissionChecker('attendanceoverviewreport')) {		
			...
			$usertype   = htmlentities(escapeString($this->uri->segment(4)));
			...
			$subjectID = 0;
			if($this->data["siteinfos"]->attendance == 'subject') {
				$subjectID  = htmlentities(escapeString($this->uri->segment(7)));
				$userID     = htmlentities(escapeString($this->uri->segment(8)));
				$monthID    = date('d-m-Y',(int)htmlentities(escapeString($this->uri->segment(9))));
				if($usertype == 1) {
					$flag = FALSE;
				}
			} else{
