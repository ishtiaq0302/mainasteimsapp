...
class Productsalereport extends Admin_Controller {
		...
		if(permissionChecker('productsalereport')) {
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
                    ...
                    {
                        $this->data['campusID']=$this->session->userdata('campus_id');
                    }else{
                        $this->data['campusID']=$this->input->post('campusID');
                    }
	...
	public function pdf() {
		if(permissionChecker('productsalereport')) {
            ...
            $productsalecustomertypeID = htmlentities(escapeString($this->uri->segment(4)));
            $productsaleclassesID      = htmlentities(escapeString($this->uri->segment(5)));
            $productsalecustomerID     = htmlentities(escapeString($this->uri->segment(6)));
            $reference_no = htmlentities(escapeString($this->uri->segment(7)));
        ...
        if((int)$productsalecustomertypeID) {
            if($productsalecustomertypeID == 1) {
                ...
                if((int)$classesID) {
                     if($campusID>0)
                    {
                        $students = $this->studentrelation_m->get_order_by_studentrelation(array('srschoolyearID' => $schoolyearID, 'srclassesID' => $classesID, 'srcampusID' => $campusID));
                    }else{
                ...
                if($campusID>0)
                {
                    $parentss = $this->parents_m->get_order_by_parents(array('campusID'=>$campusID));
                }else{
                    $parentss = $this->parents_m->get_parents();
                }
                ...
                if($campusID>0)
                {
                    $users = $this->user_m->get_order_by_user(array('usertypeID' => $productsalecustomertypeID,'campusID'=>$campusID));
                }else{
