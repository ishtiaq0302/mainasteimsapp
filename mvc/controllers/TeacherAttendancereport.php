...
class TeacherAttendancereport extends Admin_Controller {
/*
| -----------------------------------------------------
...
*/
	...
	public function index() {
		...
		// $teachers =  $this->teacher_m->general_get_order_by_teachers();
		// $query = $this->db->last->query();
		// print_r($query);
		//  exit();
		// $this->data['teachers'] = $this->tattendance_m->get_tattendance();
		...
		$this->data["subview"] = "report/attendance/TeacherAttendanceReportView";
		$this->load->view('_layout_main', $this->data);
	}

	protected function rules($teacherWise = 'teacher') {
		$rules = array(
			array(
		...
		if($teacherWise == 'teacher'){
			$rules[] = array(
				'field' => 'teacherID',
				'label' => "attendancereport_teacherid",
				'rules' => 'trim|required|xss_clean|callback_unique_data'
			);
		}
		return $rules;
		...
		if(permissionChecker('attendancereport')) {
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$day = 'a'.(int)$date[0];
					$monthyear = $date[1].'-'.$date[2];
				// 	echo json_encode($monthyear);
				// exit();
					
					// if($attendancetype) {
					if($campusID==0)
					{
						$teachers =  pluck($this->teacher_m->general_get_order_by_teacher(),'obj','teacherID');
					}else{
						$teachers =  pluck($this->teacher_m->general_get_order_by_teacher(array('campusID'=>$campusID)),'obj','teacherID');
					}
					//view($teachers,1);
					 // $teachers =  pluck($this->teacher_m->general_get_order_by_teachers(array('monthyear' => $monthyear, 'schoolyearID' => $schoolyearID)),'obj','teacherID');
				// 	 echo json_encode($teachers);
				// exit();
						$attendances =  $this->tattendance_m->get_order_by_tattendance(array('monthyear' => $monthyear, 'schoolyearID' => $schoolyearID));
				...
				// exit();
					...
					$attendances = pluck($attendances, 'obj', 'teacherID');
					...
					$retArray['render'] = $this->load->view('report/attendance/TeacherAttendanceReport', $this->data,TRUE);
	...
	public function pdf() {
		if(permissionChecker('attendancereport')) {		
			...
			if($dateFlag) {
				...
				if(!empty($classes) && $classesID > 0) {
					if(!empty($section) || $sectionID == 0) {
