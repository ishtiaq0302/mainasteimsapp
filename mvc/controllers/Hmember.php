...
*/
							...
							if ($this->form_validation->run() == FALSE) { 
								...
								if($hostel_main_id) {
									if($category_main_id) {
										...
										redirect(base_url("hmember/index/$url/".$array['campusID']));
									} else {
			...
			if((int)$id && (int)$url) {
				...
				if(isset($fetchClass[$url])) {
					...
					if(!empty($student)) {
						...
						$this->data["campusID"]=$this->data["hmember"]->campusID;
						$this->data["hostelID"]=$this->data["hmember"]->hostelID;
	...
	public function view($id = null , $url = null) {
		...
		if((int)$id && (int)$url) {
			...
			if(isset($fetchClass[$url])) {
				...
				$this->data['campusID']=$this->data['student']->srcampusID;
	...
	public function student_list() {
		...
		$campusID = $this->input->post('campusID');
		if((int)$classID) {
			$string = base_url("hmember/index/$classID/$campusID");
			echo $string;
		} else {
			redirect(base_url("hmember/index"));
		}
