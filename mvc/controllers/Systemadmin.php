...
*/
	...
	public function index() {
		...
		$this->data['systemadmins'] = $this->systemadmin_m->get_systemadmin_by_usertype(null,$campusID);
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($admindata != '')
				{
					...
					$array_meeting = array();
					$array_meeting['campusID'] = 0;
					$array_meeting['id'] = $insert_id;	
					$array_meeting['meeting_id'] = $meeting_id;	
	...
	public function edit() {
		...
		if ((int)$id) {
			...
			$this->data['campusID']=$this->data['systemadmin']->campusID;
	...
	private function getView($systemadminID,$campusID) {
		if((int)$systemadminID) {
			$systemadmin = $this->systemadmin_m->get_systemadmin_by_usertype($systemadminID,$campusID);
			$this->data['campusID']=$campusID;
			$this->pluckInfo();
	...
	public function print_preview() {
		if(permissionChecker('systemadmin_view')) {
			...
			if ((int)$id) {
				...
				if(!empty($this->data['systemadmin'])) {
					if($id != 1 && $this->session->userdata('loginuserID') != $this->data['systemadmin']->systemadminID) {
