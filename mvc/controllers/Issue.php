...
*/
	...
	public function index() {
        		...
        		$this->data['libID'] = htmlentities(escapeString($this->uri->segment(3)));
		...
		if($usertypeID == 3) {
			...
			if(!empty($student) && $student->library === '1') {
				$lmember = $this->lmember_m->get_single_lmember(array('studentID' => $student->studentID));
				$lID = $lmember->lID;
				$this->data['libraryID'] = $lID;
				$this->data['issues'] = $this->issue_m->get_issue_with_book($campusID,array("lID" => $lID));
//die('yes');
				...
				if((int)$id) {
					...
					if(!empty($checkstudent)) {
						if($checkstudent->library === '1' && 0 == 1) {
							...
							$this->data['issues'] = $this->issue_m->get_issue_with_book($campusID,array("lID" => $ulID));
							$this->data["subview"] = "issue/index";
							$this->load->view('_layout_main', $this->data);
						} else {
							$this->data['libraryID'] = 0;
	...
	public function edit() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			if((int)$id && $lID) {
				...
				$this->data['campusID']=$this->data['issue']->campusID;
				$dbGet_bookID = $this->data['issue']->bookID;
				$this->data['bookID']=$dbGet_bookID;
	...
	public function view() {
		...
		if((int)$id) {
			...
			$this->data['campusID']=$this->data['book']->campusID;
	...
	public function print_preview() {
		if(permissionChecker('issue_view')) {
			...
			if((int)$id) {
				$this->data['book'] = $this->issue_m->get_issue_with_books(array('issueID' => $id), TRUE);
				$this->data["siteinfos"] = $this->site_m->get_site_($this->data["book"]->campusID);
			...
			if($libraryID && $amount) {
				...
				if(!empty($librarymember)) {
					...
					$invoiceArray = array(
						...
						'feetypeID' => (!empty($feetype) ? $feetype->feetypesID : 0),
						'feetype' => (!empty($feetype) ? $feetype->feetypes : $this->lang->line('issue_bookfine')),
						'amount' => $amount,
						'discount' => 0,
						'paidstatus' => 0,
						'userID' => $this->session->userdata('loginuserID'),
