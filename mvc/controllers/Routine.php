...
*/
	...
	public function index() {
		...
		if((int)$id) {
			...
			$this->data['routines'] = $this->routine_m->get_routine_with_teacher_class_section_subject(array('classesID' => $id, 'schoolyearID' => $schoolyearID, 'campusID' => $campusID));
			...
			if(isset($fetchClass[$id])) {
				if($this->data['routines']) {
					...
					foreach ($sections as $key => $section) {
						$this->data['allsection'][$section->section] = $this->routine_m->get_routine_with_teacher_class_section_subject(array('classesID' => $id, 'sectionID' => $section->sectionID, 'schoolyearID' => $schoolyearID, 'campusID' => $campusID));
					}
				} else {
					$this->data['routines'] = [];
				}
	...
	public function edit() {
		...
		if((int)$id && (int)$url) {
			if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
				...
				$this->data['campusID']=$this->data['routine']->campusID;
				$this->data['classesID']=$this->data['routine']->classesID;
	...
	public function routine_list() {
		...
		$campusID = $this->input->post('campusID');
		if((int)$classID) {
			$string = base_url("routine/index/$classID/$campusID");
			echo $string;
		} else {
			redirect(base_url("routine/index"));
		}
