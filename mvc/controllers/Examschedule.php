...
*/
	...
	public function index() {
		...
		if((int)$id) {
			...
			$this->data['examschedules'] = $this->examschedule_m->get_join_examschedule_with_exam_classes_section_subject(array('classesID' => $id, 'schoolyearID' => $schoolyearID, 'campusID' => $campusID));
			if($this->data['examschedules']) {
				...
				foreach ($sections as $key => $section) {
					$this->data['allsection'][$section->section] = $this->examschedule_m->get_join_examschedule_with_exam_classes_section_subject(array('classesID' => $id, 'sectionID' => $section->sectionID, 'schoolyearID' => $schoolyearID, 'campusID' => $campusID));
				}
			} else {
		...
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					redirect(base_url("examschedule/index/".$array['classesID'].'/'.$array['campusID']));
				}
			} else {
				$this->data["subview"] = "examschedule/add";
				$this->load->view('_layout_main', $this->data);
		...
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			if((int)$id && (int)$url) {
				...
				if(isset($fetchClass[$url])) {
					...
					$this->data['campusID']=$this->data['examschedule']->campusID;
					$this->data['classesID']=$this->data['examschedule']->classesID;
					$this->data['examID']=$this->data['examschedule']->examID;
					if(!empty($this->data['examschedule'])) {
	...
	public function examschedule_list() {
		...
		$campusID = $this->input->post('campusID');
		if((int)$classID) {
			$string = base_url("examschedule/index/$classID/$campusID");
			echo $string;
		} else {
			redirect(base_url("examschedule/index"));
		}
