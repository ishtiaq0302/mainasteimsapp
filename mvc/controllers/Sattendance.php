...
*/
				...
				if(isset($fetchClass[$id])) {
					if(!empty($this->data['students'])) {
						...
						foreach ($sections as $key => $section) {
							$this->data['allsection'][$section->section] = $this->studentrelation_m->get_order_by_student(array('srclassesID' => $id, "srsectionID" => $section->sectionID, 'srschoolyearID' => $schoolyearID, 'srcampusID' => $campusID));
						}
					} else {
						$this->data['students'] = [];
					}
	...
	public function student_list() {
		...
		$campusID = $this->input->post('campusID');
		if((int)$classID) {
			$string = base_url("sattendance/index/$classID/$campusID");
			echo $string;
		} else {
	...
	private function sendAbsentEmail($students, $schoolyearID, $classesID, $sectionID) {
		...
		foreach($students as $student) {
			...
			if(!empty($user) && $parentsID > 0 && $parentsEmail != '') {
	...
	private function sendAbsentSMS($students, $schoolyearID, $classesID, $sectionID) {
		...
		foreach($students as $student) {
			...
			if(!empty($user) && $parentsID > 0 && $parentsPhonenumber != '') {
	...
	public function view($id = null, $url = null) {
		...
		if((int)$id && (int)$url) {
			...
			$this->data["campusID"]=$this->data["student"]->campusID;
			$this->data["classes"] = $this->classes_m->get_single_classes(array('classesID' => $url));
			if(!empty($this->data["student"]) && !empty($this->data["classes"])) {
				$this->data['set'] = $url;
