...
*/
	...
	public function index() {
		...
		$this->data['activitiescategorys'] = $this->activitiescategory_m->get_order_by_activitiescategory(array('campusID'=>$campusID));
	...
	public function add() {
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url("activitiescategory/index/".$array['campusID']));
			}
		} else {
			$this->data["subview"] = "activitiescategory/add";
			$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		...
		if((int)$id) {
			...
			$this->data['campusID']=$this->data['activitiescategory']->campusID;
	...
	public function unique_activitiescategory() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$activitiescategory = $this->activitiescategory_m->get_order_by_activitiescategory(array("title" => $this->input->post("title"), "activitiescategoryID !=" => $id, "campusID" => $this->input->post("campusID")));
			if(!empty($activitiescategory)) {
				$this->form_validation->set_message("unique_activitiescategory", "%s already exists");
				return FALSE;
			}
			return TRUE;
		} else {
			$activitiescategory = $this->activitiescategory_m->get_order_by_activitiescategory(array("title" => $this->input->post("title"), "campusID" => $this->input->post("campusID")));
			if(!empty($activitiescategory)) {
				$this->form_validation->set_message("unique_activitiescategory", "%s already exists");
				return FALSE;
			}
