...
*/
	...
	public function index() {
		...
		$this->data['transports'] = $this->transport_m->get_order_by_transport(array('campusID'=>$campusID));
	...
	public function add() {
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url("transport/index/".$array['campusID']));
			}
		} else {
			$this->data["subview"] = "transport/add";
			$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		...
		if((int)$id) {
			...
			$this->data['campusID'] = $this->data['transport']->campusID;
	...
	function unique_route() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$transport = $this->transport_m->get_order_by_transport(array("route" => $this->input->post("route"), "transportID !=" => $id, "campusID" => $this->input->post("campusID")));
			if(!empty($transport)) {
			...
			$transport = $this->transport_m->get_order_by_transport(array("route" => $this->input->post("route"), "campusID" => $this->input->post("campusID")));
