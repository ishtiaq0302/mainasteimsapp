...
*/
	...
	public function index() {
		...
		$this->data['grades'] = $this->grade_m->get_order_by_grade(array('campusID'=>$campusID));
	...
	public function add() {
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url("grade/index/".$array['campusID']));
			}
		} else {
			$this->data["subview"] = "grade/add";
			$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		...
		if((int)$id) {
			...
			$this->data['campusID']=$this->data['grade']->campusID;
			if($this->data['grade']) {
				if($_POST) {
					$rules = $this->rules();
					$this->form_validation->set_rules($rules);
	...
	public function unique_grade() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$grade = $this->grade_m->get_order_by_grade(array("grade" => $this->input->post("grade"), "gradeID !=" => $id, "campusID" => $this->input->post("campusID")));
			if(!empty($grade)) {
			...
			$grade = $this->grade_m->get_order_by_grade(array("grade" => $this->input->post("grade"), "campusID" => $this->input->post("campusID")));
	...
	public function unique_point() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$grade = $this->grade_m->get_order_by_grade(array("point" => $this->input->post("point"), "gradeID !=" => $id, "campusID" => $this->input->post("campusID")));
			if(!empty($grade)) {
			...
			$grade = $this->grade_m->get_order_by_grade(array("point" => $this->input->post("point"), "campusID" => $this->input->post("campusID")));
	...
	public function unique_gradefrom() {
		...
		if((int)$id) {
			if($this->input->post("gradefrom") != '') {
				$grade = $this->grade_m->get_order_by_grade(array("gradefrom" => $this->input->post("gradefrom"), "gradeID !=" => $id, "campusID" => $this->input->post("campusID")));
				if(!empty($grade)) {
			...
			if($this->input->post("gradefrom") != '') {
				$grade = $this->grade_m->get_order_by_grade(array("gradefrom" => $this->input->post("gradefrom"), "campusID" => $this->input->post("campusID")));
				if(!empty($grade)) {
					$this->form_validation->set_message("unique_gradefrom", "%s already exists");
					return FALSE;
				}
	...
	public function unique_gradeupto() {
		...
		if((int)$id) {
			if($this->input->post("gradeupto") != '') {
				$grade = $this->grade_m->get_order_by_grade(array("gradeupto" => $this->input->post("gradeupto"), "gradeID !=" => $id, "campusID" => $this->input->post("campusID")));
				if(!empty($grade)) {
			...
			if($this->input->post("gradeupto") != '') {
				$grade = $this->grade_m->get_order_by_grade(array("gradeupto" => $this->input->post("gradeupto"), "campusID" => $this->input->post("campusID")));
				if(!empty($grade)) {
					$this->form_validation->set_message("unique_gradeupto", "%s already exists");
					return FALSE;
				}
