...
*/
	...
	public function index() {
		...
		$this->data['feetypes'] = $this->feetypes_m->get_order_by_feetypes(array('campusID' => $campusID));
	...
	public function add() {
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url("feetypes/index/".$array['campusID']));
			}
		} else {
			$this->data["subview"] = "feetypes/add";
			$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		...
		if((int)$id) {
			...
			$this->data['campusID'] = $this->data['feetypes']->campusID;
	...
	public function unique_feetypes() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$feetypes = $this->feetypes_m->get_order_by_feetypes(array("feetypes" => $this->input->post("feetypes"), "feetypesID !=" => $id, "campusID" => $this->input->post("campusID")));
			if(!empty($feetypes)) {
			...
			if($monthly) {
				...
				$feetypes = $this->feetypes_m->get_order_by_feetypes(array("feetypes" => $this->input->post("feetypes"), "campusID" => $this->input->post("campusID")));
				if(!empty($feetypes)) {
					$this->form_validation->set_message("unique_feetypes", "%s already exists");
					return FALSE;
				}
