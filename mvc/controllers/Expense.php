...
*/
	...
	public function index() {
		...
		$this->data['expenses'] = $this->expense_m->get_order_by_expense(array('schoolyearID' => $schoolyearID,'campusID' => $campusID));
		...
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) {
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					redirect(base_url("expense/index/".$array['campusID']));
				}
			} else {
				$this->data["subview"] = "expense/add";
				$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) {
			...
			if((int)$id) {
				...
				$this->data['campusID']=$this->data['expense']->campusID;
