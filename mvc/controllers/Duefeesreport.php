...
class Duefeesreport extends Admin_Controller{
		...
		if(permissionChecker('duefeesreport')) {
			if($_POST) {
			    ...
			    if ($this->form_validation->run() == FALSE) {
					...
					if((int)$campusID) {
						$studnetArray['srcampusID'] = $campusID;
					}
					if((int)$sectionID) {
	...
	public function pdf() {
		if(permissionChecker('duefeesreport')) { 
			...
			$feetypeID = htmlentities(escapeString($this->uri->segment(7)));
	...
	private function xmlData() {
		if(permissionChecker('duefeesreport')) { 
			...
			if((int)($classesID >= 0) || (int)($sectionID >= 0) || (int)($studentID >= 0) || (int)($feetypeID >= 0) || ((int)($fromdate >= 0) || (int)($fromdate =='') || ((int)($todate >= 0) || (int)($todate == '')))) {
				...
				if(!empty($this->data['getDueFeesReports'])) {
	...
	private function generateXML($data) {
		...
		if(!empty($getDueFeesReports)) {
	        ...
	        //Make Here Xml Body
	        $row  = 3;
	        if(!empty($getDueFeesReportArrays)) {
