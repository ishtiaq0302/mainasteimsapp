...
class Librarybooksreport extends Admin_Controller{
	...
	public function pdf() {
		if(permissionChecker('librarybooksreport')) {
			...
			$bookname    = htmlentities(escapeString($this->uri->segment(4)));
			$subjectcode = htmlentities(escapeString($this->uri->segment(5)));
			$rackNo  = htmlentities(escapeString($this->uri->segment(6)));
        	...
        	if(((int)$bookname >= 0) && (((int)$subjectcode >= 0) || (string)$subjectcode) && (((int)$rackNo >= 0) || (string)$rackNo) && ((int)$status >= 0)) {
				...
				if($this->data['campusID'] != '0') {
					$postArray['campusID'] = $this->data['campusID'];
				}
