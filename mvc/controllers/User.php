...
*/
	...
	public function index() {
		...
		if(isset($usertype[$this->session->userdata('usertypeID')]) && $myProfile) {
			...
			$this->data['users'] = $this->user_m->get_user_by_usertypes($campusID);
	...
	public function add() {
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url("user/index/".$array['campusID']));
			}
		} else {
			$this->data["subview"] = "user/add";
			$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		...
		if((int)$id) {
			...
			$this->data['campusID']=$this->data['user']->campusID;
			$this->data['usertypeID']=$this->data['user']->usertypeID;
			if($this->data['user']) {
				$rules = $this->rules();
				unset($rules[12]);
				$this->form_validation->set_rules($rules);
	...
	private function getView($userID) {
		if ((int)$userID) {
			...
			$this->data['campusID']=$users->campusID;
			$this->pluckInfo();
	...
	public function print_preview() {
		...
		if(permissionChecker('user_view') || ((isset($usertype[$this->session->userdata('usertypeID')])) && permissionChecker('user') && ($this->session->userdata('loginuserID') == htmlentities(escapeString($this->uri->segment(3)))))) {
			...
			if((int)$userID) {
				...
				$this->data["siteinfos"] = $this->site_m->get_site_($this->data["user"]->campusID);
