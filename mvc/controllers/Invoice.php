...
*/
    ...
    public function index() {
		...
		if($usertypeID == 3) {
			...
			if(!empty($student)) {
				$this->data['maininvoices'] = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_studentIDs($student->studentID, $schoolyearID,$campusID);
			...
			$students = $this->studentrelation_m->get_order_by_student(array('parentID' => $parentID, 'srschoolyearID' => $schoolyearID, 'srcampusID' => $campusID));
			if(!empty($students)) {
				$studentArray = pluck($students, 'srstudentID');
				$this->data['maininvoices'] = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_multi_studentIDs($studentArray, $schoolyearID,$campusID);
				$this->data['grandtotalandpayment'] = $this->grandtotalandpaid($this->data['maininvoices'], $schoolyearID);
				$this->data["subview"] = "invoice/index";
				$this->load->view('_layout_main', $this->data);
			} else {
			...
			$this->data['maininvoices'] = $this->maininvoice_m->get_maininvoice_with_studentrelations($schoolyearID,$campusID);
	...
	public function edit() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('defaultschoolyearID') == 5)) {
			...
			if((int)$maininvoiceID) {
				...
				$this->data['getinvoice'] = $this->invoice_m->get_single_invoice(array('maininvoiceID' => $maininvoiceID));

				$this->data['campusID']=$this->data['getinvoice']->campusID;
				$this->data['classesID']=$this->data['getinvoice']->classesID;
				$this->data['feetypeID']=$this->data['getinvoice']->feetypeID;

				if(!empty($this->data['maininvoice'])) {
        ...
        if($usertypeID == 3) {
            ...
            if((int)$id) {
                ...
                if(!empty($getstudent)) {
	                ...
	                $this->data['campusID']=$this->data['maininvoice']->campusID;
	                $this->data["siteinfos"] = $this->site_m->get_site_($this->data['campusID']);
	                if(!empty($this->data['maininvoice']) && ($this->data['maininvoice']->maininvoicestudentID == $getstudent->studentID)) {
						...
						$this->data["parent"] = $this->parents_m->get_single_parents(array('parentsID'=>$this->data["student"]->parentID));	
        ...
        if(permissionChecker('invoice_view')) {
            ...
            if($usertypeID == 3) {
	            ...
	            if((int)$id) {
	                ...
	                if(!empty($getstudent)) {
		                $this->data['maininvoice'] = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_maininvoiceID($id, $schoolyearID);
	                $this->data["siteinfos"] = $this->site_m->get_site_($this->data['maininvoice']->campusID);
		                if(!empty($this->data['maininvoice']) && ($this->data['maininvoice']->maininvoicestudentID == $getstudent->studentID)) {
			                $this->data['invoices'] = $this->invoice_m->get_order_by_invoice(array('maininvoiceID' => $id));
    ...
    public function viewpayment() {
    	if(permissionChecker('invoice_view')) {
	    	...
	    	if((int)$globalpaymentID && (int)$maininvoiceID) {
	    		...
	    		$this->data["siteinfos"] = $this->site_m->get_site_($globalpayment->campusID);
	    		...
	    		if(!empty($maininvoice) && !empty($globalpayment)) {
	    			$usertypeID = $this->session->userdata('usertypeID');
	    			$userID = $this->session->userdata('loginuserID');
					                    		...
					                    		foreach ($feetypeitems as $feetypeitem) {
													$invoiceArray[] = array(
														'schoolyearID' => $invoiceMainArray[$j]['maininvoiceschoolyearID'],
						                         		...
						                         		'campusID' => $invoiceMainArray[$j]['campusID'],
        ...
        if(!empty($maininvoices)) {
        	foreach ($maininvoices as $maininvoice) {
        		if(isset($invoiceitems[$maininvoice->maininvoiceID])) {
        			if(!empty($invoiceitems[$maininvoice->maininvoiceID])) {
        				foreach ($invoiceitems[$maininvoice->maininvoiceID] as $invoiceitem) {
        					...
        					if(isset($paymentitems[$invoiceitem->invoiceID])) {
        						if(!empty($paymentitems[$invoiceitem->invoiceID])) {
        					...
        					if(isset($weaverandfineitems[$invoiceitem->invoiceID])) {
        						if(!empty($weaverandfineitems[$invoiceitem->invoiceID])) {
