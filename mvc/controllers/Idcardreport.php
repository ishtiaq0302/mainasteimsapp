...
*/
    ...
    public function getUser() {
        ...
        if((int)$usertypeID && ((int)$classesID || $classesID == 0) && ((int)$sectionID || $sectionID ==0)) {
            ...
            if($usertypeID == 1) {
                if($campusID>0){
                     $users = $this->systemadmin_m->get_order_by_systemadmin(array('campusID'=>$campusID));
                }else{
                ...
                if($campusID>0){
                    $users = $this->teacher_m->general_get_order_by_teacher(array('campusID'=>$campusID));
                }else{
    ...
    private function queryArray($posts) {
        ...
        {
            $campusID     = $this->session->userdata('campus_id');
            $queryArray['campusID'] = $campusID;
        }else{
            $campusID     = $posts['campusID'];
            if($campusID>0){
                $queryArray['campusID'] = $campusID;
            }
        }
    ...
    public function pdf() {
        if(permissionChecker('idcardreport')) {
            ...
            $type         = htmlentities(escapeString($this->uri->segment(8)));
            $background   = htmlentities(escapeString($this->uri->segment(9)));
            $schoolyearID = $this->session->userdata('defaultschoolyearID');
