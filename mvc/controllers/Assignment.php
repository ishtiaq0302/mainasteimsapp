...
*/
	...
	public function index() {
		...
		if((int)$id) {
			if(isset($fetchClasses[$id])) {
				...
				$this->data['assignments'] = $this->assignment_m->join_get_assignment($id, $schoolyearID, $campusID);
		...
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) { 
					...
					redirect(base_url("assignment/index/".$array['classesID'].'/'.$array['campusID']));
				}
			} else {
				$this->data["subview"] = "assignment/add";
				$this->load->view('_layout_main', $this->data);
		...
		if((int)$id && (int)$url) {
			if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
				...
				if(isset($fetchClasses[$url])) {
					...
					$this->data['campusID']=$this->data['assignment']->campusID;
					$this->data['classesID']=$this->data['assignment']->classesID;
					$this->data['usertypeID']=$this->data['assignment']->usertypeID;
					if($this->data['assignment']) {
						...
						if($_POST) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$this->session->set_flashdata('success', $this->lang->line('menu_success'));
								redirect(base_url("assignment/index/$url".'/'.$array['campusID']));
							}
						} else {
							$this->data["subview"] = "assignment/edit";
							$this->load->view('_layout_main', $this->data);
	...
	public function view() {
		...
		if((int)$id && (int)($url)) {
			...
			if(isset($fetchClasses[$url])) {
				...
				$assignment = $this->assignment_m->get_single_assignment(array('assignmentID' => $id, 'classesID' => $url, 'schoolyearID' => $schoolyearID));
				$this->data['campusID']=$assignment->campusID;
				if(!empty($assignment)) {
	...
	public function student_list() {
		...
		$campusID = $this->input->post('campusID');
		if((int)$classID) {
			$string = base_url("assignment/index/$classID/$campusID");
			echo $string;
		} else {
			redirect(base_url("assignment/index"));
		}
