...
class Hostel extends Admin_Controller {
	...
	public function index() {
		...
		$this->data['hostels'] = $this->hostel_m->get_order_by_hostel(array('campusID'=>$campusID));
	...
	public function add() {
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url("hostel/index/".$array['campusID']));
			}
		} else {
			$this->data["subview"] = "hostel/add";
			$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		...
		if((int)$id) {
			...
			$this->data['campusID']=$this->data['hostel']->campusID;
			if($this->data['hostel']) {
				if($_POST) {
					$rules = $this->rules();
					$this->form_validation->set_rules($rules);
	...
	function unique_name() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$student = $this->hostel_m->get_order_by_hostel(array("name" => $this->input->post("name"), "hostelID !=" => $id, "campusID" => $this->input->post("campusID")));
			if(!empty($student)) {
			...
			$student = $this->hostel_m->get_order_by_hostel(array("name" => $this->input->post("name"), "campusID" => $this->input->post("campusID")));
			if(!empty($student)) {
				$this->form_validation->set_message("unique_name", "%s already exists");
				return FALSE;
			}
