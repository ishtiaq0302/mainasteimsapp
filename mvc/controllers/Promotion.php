...
*/
	...
	public function index() {
		...
		if((int)$id && (int)$schoolyearID) {
			...
			$this->data['section'] = $this->section_m->general_get_order_by_section(array('classesID' => $id,'campusID' => $campusID));
			...
			if ($_POST) {
				...
				$sectionsID = $this->input->post('sectionID');
				if ($sectionsID == 0) {
					redirect("promotion/index/$id/$schoolyearID/$campusID/$sectionID");
				}
				
				$jsectionID = $this->input->post('jsectionID');
				if($jsectionID==0){
					$this->session->set_flashdata('error', 'Please Select Promotion Section');
					echo 'error';
					redirect("promotion/index/$id/$schoolyearID/$campusID/$sectionID");
				}
				...
				$promotionLog = array(
					...
					'jumpSectionID' => $this->input->post('jsectionID'),
				...
				redirect("promotion/add/$id/$schoolyearID/$campusID/$sectionsID");
			} else {
	...
	public function promotion_list() {
		...
		if((int)$classID) {
			$string = base_url("promotion/index/$classID/$schoolyearID/$campusID/$sectionID");
			echo $string;
		} else {
	...
	public function add() {
		...
		if((int)$classID && (int)$schoolyearID) {
			...
			if(!empty($classes) && !empty($schoolyear)) {
				...
				$this->data['schoolyearID'] = $schoolyearID;
				$this->data['sections']	= pluck($this->section_m->general_get_order_by_section(array('classesID' => $classID,'sectionID' => $sectionID)), 'obj', 'sectionID');
				$this->studentPromotionCalculation($classID, $schoolyearID,$campusID,$sectionID);
	...
	private function studentPromotionCalculation($classID, $schoolyearID, $campusID, $sectionID) {
		$marks		= $this->mark_m->get_order_by_student_mark_with_subject($classID, $schoolyearID);
		$students	= $this->student_m->general_get_order_by_student(array('classesID' => $classID, 'schoolyearID' => $schoolyearID, 'campusID' => $campusID, 'sectionID' => $sectionID));
		...
		foreach ($marks as $key => $value) {
			...
			//view($promotionMarkPercentages,1);
	...
	public function summary() {
		...
		if((int)$studentID && (int)$classID && (int)$schoolyearID) {
			...
			if(!empty($checkStudent) && !empty($checkClass) && !empty($checkSchoolyear)) {
				$this->data['set'] = $classID;
	...
	public function promotion_to_next_class() {
		...
		$previousSectionID = $promotionLog->sectionID;
		$previousYearID = $promotionLog->schoolYearID;
		
		$promotionClassID = $promotionLog->jumpClassID;
		$promotionSectionID = $promotionLog->jumpSectionID;
		$promotionYearID = $promotionLog->jumpSchoolYearID;

		$explodeStudents = explode(",",  $studentIDs);
		$students = pluck($this->student_m->general_get_order_by_student(array("classesID" => $previousClasseID,"sectionID" => $previousSectionID, "schoolyearID" => $previousYearID)), 'obj', 'studentID');

		$promoteClassPreviousStudentsList = pluck($this->student_m->general_get_order_by_student(array("classesID" => isset($enroll) && $enroll ? $previousClasseID : $promotionClassID, "schoolyearID" => $promotionYearID, "sectionID" => isset($enroll) && $enroll ? $previousSectionID : $promotionSectionID)), 'obj', 'studentID');

		$sections =$this->section_m->general_get_order_by_section(array("classesID" => isset($enroll) && $enroll ? $previousClasseID : $promotionClassID, "sectionID" => isset($enroll) && $enroll ? $previousSectionID : $promotionSectionID));
		...
		if(!empty($students) && !empty($studentIDs) && !empty($previousClasseID)) {
			$f=0;
			...
			foreach ($explodeStudents as $key => $studentID) {
				...
				if(isset($students[$studentID])) {
					...
					$array = array(
						...
						'sectionID' => isset($enroll) && $enroll ? $previousSectionID : $promotionSectionID,
					);
					...
					$sectionRelation = $this->section_m->general_get_section($promotionSectionID);*/

					$classesRelation = $this->classes_m ->general_get_classes(isset($enroll) && $enroll ? $previousClasseID : $promotionClassID);
					$sectionRelation = $this->section_m->general_get_section(isset($enroll) && $enroll ? $previousSectionID : $promotionSectionID);
					...
					$promoteStudents[] = array('studentID' => $studentID, 'roll' => $roll, 'enroll' => $enroll, 'sectionID' => $promotionSectionID);
