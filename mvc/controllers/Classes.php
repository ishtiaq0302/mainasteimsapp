...
*/
	...
	public function index() {
		...
		$this->data['classes'] = $this->classes_m->get_order_by_classes(array('campusID' => $campusID));
	...
	public function add() {
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) { 
				...
				redirect(base_url("classes/index/".$array['campusID']));
			}
		} else {
			$this->data["subview"] = "classes/add";
			$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		...
		if((int)$id) {
			...
			$this->data['campusID']=$this->data['classes']->campusID;
			$this->data['teacherID']=$this->data['classes']->teacherID;
	...
	public function unique_classes() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$classes = $this->classes_m->get_order_by_classes(array("classes" => $this->input->post("classes"), "classesID !=" => $id, "campusID" => $this->input->post("campusID")));
			if(!empty($classes)) {
			...
			$classes = $this->classes_m->get_order_by_classes(array("classes" => $this->input->post("classes"),"campusID" => $this->input->post("campusID")));
			if(!empty($classes)) {
					$this->form_validation->set_message("unique_classes", "%s already exists");
					return FALSE;
			}
	...
	public function unique_classes_numeric() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$classes_numeric = $this->classes_m->get_order_by_classes(array("classes_numeric" => $this->input->post("classes_numeric"), "classesID !=" => $id, "campusID" => $this->input->post("campusID")));
			if(!empty($classes_numeric)) {
				$this->form_validation->set_message("unique_classes_numeric", "%s already exists");
				return FALSE;
			}
			return TRUE;
		} else {
			$classes_numeric = $this->classes_m->get_order_by_classes(array("classes_numeric" => $this->input->post("classes_numeric"),"campusID" => $this->input->post("campusID")));
			if(!empty($classes_numeric)) {
					$this->form_validation->set_message("unique_classes_numeric", "%s already exists");
					return FALSE;
			}
