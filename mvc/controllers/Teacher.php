...
*/
	...
	public function index() {
		...
		if($this->session->userdata('usertypeID') == 2 && $myProfile) {
			...
			$this->data['teachers'] = $this->teacher_m->get_order_by_teacher(array('campusID'=>$campusID));
  ...
  //   exit();
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$array['meeting_id'] = $this->input->post("meeting_id");
			 ...
			 $teacherdata = $this->teacher_m->insert_teacher($array);
			$insert_id = $this->db->insert_id();
			
				if($teacherdata != '')
				{
					$array_meeting = array();
				$array_meeting['id'] = $insert_id;	
				$array_meeting['campusID'] = $this->input->post("campusID");	
				$array_meeting['meeting_id'] = $this->input->post("meeting_id");	
				$array_meeting['name'] = $this->input->post("name");
				$array_meeting['email'] = $this->input->post("email");
				$array_meeting['phone'] = $this->input->post("phone");
				$array_meeting['address'] = $this->input->post("address");
				$array_meeting['jod'] = date("Y-m-d", strtotime($this->input->post("jod")));
				$array_meeting['username'] = $this->input->post("username");
				$array_meeting['password'] = $this->teacher_m->hash($this->input->post("password"));
				$array_meeting['usertypeID'] = 2;
				$array_meeting["create_date"] = date("Y-m-d h:i:s");
				$array_meeting["modify_date"] = date("Y-m-d h:i:s");
				$array_meeting["create_userID"] = $this->session->userdata('loginuserID');
				$array_meeting["create_username"] = $this->session->userdata('username');
				$array_meeting["create_usertype"] = $this->session->userdata('usertype');
				$array_meeting["active"] = 1;
				$this->staff_m->insert_staff($array_meeting);
				}
				
				$this->session->set_flashdata('success', $this->lang->line('menu_success'));
				redirect(base_url("teacher/index/".$array['campusID']));
			}
		} else {
			$this->data["subview"] = "teacher/add";
			$this->load->view('_layout_main', $this->data);
	...
	public function edit() {
		...
		if((int)$id) {
			...
			$this->data['campusID']=$this->data['teacher']->campusID;
			if($this->data['teacher']) {
				if($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$array_staff = array();
						$array_staff['campusID'] = $this->input->post("campusID");
						$this->staff_m->update_staff($array_staff, $id);
	...
	private function getView($teacherID) {
		if((int)$teacherID) {
			...
			$this->data['campusID']=$teacherinfo->campusID;
			$this->pluckInfo();
            ...
            if(!empty($manageSalary)) {
                ...
                if($manageSalary->salary == 1) {
                    ...
                    if($this->data['salary_template']) {
                        ...
                        if(!empty($this->data['salaryoptions'])) {
                    ...
                    if(!empty($this->data['hourly_salary'])) {
                        $this->data['grosssalary'] = 0;
                        $this->data['totaldeduction'] = 0;
	...
	public function print_preview() {
		if(permissionChecker('teacher_view') || (($this->session->userdata('usertypeID') == 2) && permissionChecker('teacher') && ($this->session->userdata('loginuserID') == htmlentities(escapeString($this->uri->segment(3)))))) {
			...
			if ((int)$id) {
				$this->data["teacher"] = $this->teacher_m->get_single_teacher(array('teacherID' => $id));
				$this->data["siteinfos"] = $this->site_m->get_site_($this->data["teacher"]->campusID);
