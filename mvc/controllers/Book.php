...
*/
	...
	public function index() {
		...
		$this->data['books'] = $this->book_m->get_order_by_book(array('campusID'=>$campusID));
	...
	public function add() {
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect(base_url("book/index/".$array['campusID']));
			}
		} else {
			$this->data["subview"] = "book/add";
			$this->load->view('_layout_main', $this->data);
	...
	public function unique_book() {
		$id = htmlentities(escapeString($this->uri->segment(3)));
		if((int)$id) {
			$student = $this->book_m->get_order_by_book(array("book" => $this->input->post("book"), "bookID !=" => $id, "author" => $this->input->post('author'), "subject_code" => $this->input->post('subject_code'), "campusID" => $this->input->post("campusID")));
			if(!empty($student)) {
			...
			$student = $this->book_m->get_order_by_book(array("book" => $this->input->post("book"), "author" => $this->input->post('author'), "subject_code" => $this->input->post('subject_code'), "campusID" => $this->input->post("campusID")));
