...
class Childcare_m extends MY_Model {
	...
	public function get_join_childcare_all($schoolyearID = NULL, $id = NULL, $campusID = NULL) {
        ...
        if((int)$schoolyearID) {
        	...
        	$this->db->where('childcare.campusID', $campusID);

        $this->db->where('childcare.adminID',$this->session->userdata('adminID'));
