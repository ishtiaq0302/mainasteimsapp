...
class student_m extends MY_Model {
	...
	public function general_get_student($array=NULL, $signal=FALSE) {
		...
		$this->db->join('studentextend', 'studentextend.studentID = student.studentID', 'LEFT');
        	$this->db->where('studentextend.adminID',$this->session->userdata('adminID'));
	...
	public function get_student($id=NULL, $single=FALSE) {
		...
		if($usertypeID == 2) {
        	...
        	$this->db->where('student.adminID',$this->session->userdata('adminID'));
	...
	public function get_auto_register_no($campusID=0) {
        $data = array();
		
		$this->db->where('adminID',$this->session->userdata('adminID'));
		
		if($campusID==0){
			if($this->session->userdata('campus_id')==1){
				$this->db->where('campusID',$this->session->userdata('accountCampusID'));
			}else{
				$this->db->where('campusID',$this->session->userdata('campus_id'));				
			}
		}else{
        ...
        if ($q->num_rows() > 0) {
			$rows = $q->row_array(); 
			$data = (int)$rows['s_registration_no_auto'];
        }
		else{
			$data = 0;
		}
    ...
    /*public function get_auto_register_no() {
        $data = array();
        $this->db->limit(1);
		$this->db->where('fieldoption','s_registration_no_auto');
        $q = $this->db->get('setting');
        if ($q->num_rows() > 0) {
			$rows = $q->result_array(); 
			$data = (int)$rows[0]['value'];
        }
		else{
			$data = 0;
		}
	...
	public function update_auto_register_no($array){
		$this->db->set('s_registration_no_auto', 's_registration_no_auto+1',FALSE);
		$this->db->where('campusID',$array['campusID']);
