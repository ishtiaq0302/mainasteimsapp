...
class Payment_m extends MY_Model {
	...
	public function get_payment_with_studentrelation_by_studentID($studentID, $schoolyearID,$campusID=0) {
		...
		$this->db->where('payment.adminID',$this->session->userdata('adminID'));



		$this->db->where(array('payment.schoolyearID' => $schoolyearID));

		$this->db->where(array('payment.campusID' => $campusID));
	...
	public function get_payment_with_studentrelation($schoolyearID,$campusID=0) {
	...
	public function get_all_payment_for_report($queryArray) {
		...
		if(isset($queryArray['campusID']) && $queryArray['campusID'] != 0) {

				$this->db->where('payment.campusID', $queryArray['campusID']);
	...
	public function get_payments($array) {
		...
		$this->db->where('payment',$this->session->userdata('adminID'));

		if($array['campusID']>0)

		{

			$this->db->where('payment.campusID',$array['campusID']);
	...
	public function get_single_payment_by_globalpaymentID($globalpaymentID) {
        ...
        $this->db->where('globalpayment.adminID',$this->session->userdata('adminID'));
