...
class Signin_m extends MY_Model {
	...
	public function signin() {
		...
		$adminID=0;
		if(isset($userdata->systemadminID) != '')
		{
			$adminID=$userdata->systemadminID;
			$accountCampusID=$userdata->accountCampusID;
			$adminLogin=1;
		}else{
			if(empty($userdata))
			{
		...
		if($accountCampusID==0)
		{
			if(empty($userdata))
			{
				$getcampusID = 0;
			}else{
				$getcampusID = $userdata->campusID;
			}

		}else{
			$getcampusID = $accountCampusID;			
		}

		$settings = $this->setting_m->get_setting($getcampusID,$adminID);

		if(empty($settings))
		{
			$lang = 0;
			$defaultschoolyearID = 0;
		}else{
				...
				if(!empty($usertype)) {
					if($userdata->active == 1 || $userdata->active == 0) {
						...
						$data = array(
							...
							"meeting_id" 			=> $userdata->meeting_id,
							"zoom_api_key" 			=> isset($userdata->zoom_api_key),
							"zoom_api_secret" 		=> isset($userdata->zoom_api_secret),
