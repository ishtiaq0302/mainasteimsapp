        ...
        <div class="box">
            ...
            <div class="box-body">
                <form role="form" method="post" enctype="multipart/form-data" id="invoiceDataForm"> 
                    <?php getCampus($campusID,12,12,12); ?>
                                ...
                                if(!empty($invoices)) { 
                                    ...
                                    foreach ($invoices as $invoice) {
                                        ...
                                        echo '<tr id="tr_'.$randID.'" invoicefeetypeID="'.$invoice->feetypeID.'">';
                                            ...
                                            echo '<td>';
												echo '<input type="text" class="form-control change_discount_p" id="p_discount_id_'.$randID.'" data-discount-id="'.$randID.'"  value="'.$discount.'" >';
                                                echo '<input type="hidden" class="form-control change-discount" id="td_discount_id_'.$randID.'" data-discount-id="'.$randID.'" value="'.$invoice->discount.'">';
                                            echo '</td>';
...
<script type="text/javascript">
    ...
    function productItemDesign(feetypeID, productText) {
        ...
        var text = '<tr id="tr_'+randID+'" invoicefeetypeID="'+feetypeID+'">';
            ...
            text += '<td>';
                text += ('<input type="text" class="form-control change_discount_p" id="p_discount_id_'+randID+'" data-discount-id="'+randID+'">');
                text += ('<input type="hidden" class="form-control change-discount" id="td_discount_id_'+randID+'" data-discount-id="'+randID+'">');
            text += '</td>';
    ...
    function totalInfo() {
        ...
        $('#feetypeList tr').each(function(index, value) {
            if($(this).children().eq(3).children().val() != '' && $(this).children().eq(3).children().val() != null && $(this).children().eq(3).children().val() != '.') {
                var discount = parseFloat($(this).children().eq(3).find('.change_discount_p').val());
                totalDiscount += discount;
            } 
        });
        ...
        $('#feetypeList tr').each(function(index, value) {
            var amount = parseFloat($(this).children().eq(2).children().val());
            var discount = parseFloat($(this).children().eq(3).find('.change-discount').val());
            var subtotal = 0;
            if(amount > 0) {
                if(discount > 0) {
                    if(discount == 100) {
    ...
    $(document).on('keyup', '.change-amount', function() {
        ...
        var p_dis = $(this).parent().parent().find('.change_discount_p').val();
		if(p_dis != '' && p_dis != null){
			var dis = (p_dis/amount)*100;
			$(this).parent().parent().find('.change-discount').val(dis);
		}
	...
	$(document).on('keyup', '.change_discount_p', function() {
		var trID = $(this).parent().parent().attr('id').replace('tr_','');
		...
		$( "#td_discount_id_"+randID ).trigger( "keyup" );
	});
    ...
    $(document).on('keyup', '.change-discount', function() {
		...
		var p_dis = $(this).parent().find('.change_discount_p').val();
		if(p_dis != '' && p_dis != null){
			var dis = (p_dis/amount)*100;
			$(this).val(dis);
		}
...
<script type="text/javascript">
    ...
    $(document).on('click', '#editInvoiceButton', function() {
        ...
        var feetypeitems = $('tr[id^=tr_]').map(function(){
            ...
            return { feetypeID : $(this).attr('invoicefeetypeid'), amount: $(this).children().eq(2).children().val(), discount : $(this).children().eq(3).find('.change-discount').val(), subtotal: $(this).children().eq(4).text() , paidamount: $(this).children().eq(5).children().val() };
        }).get();
...
function getInfoByCampus(campusID)
{
    $.ajax({
        ...
        success: function(data) {
            $('#classesID').html(data);
            $('#classesID option[value="<?=$classesID?>"]').attr('selected','selected');
        }
    });
    ...
    $.ajax({
        async: false,
        type: 'POST',
        url: "<?=base_url('universalfunction/feetypecall')?>",
        data: "id=" + campusID,
        dataType: "html",
        success: function(data) {
            $('#feetypeID').html(data);
            $('#feetypeID option[value="<?=$feetypeID?>"]').attr('selected','selected');
        }
    });
}    
</script>