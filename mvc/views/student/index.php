                                                ...
                                                <tr>
                                                    ...
                                                    <?php if(permissionChecker('student_edit') || permissionChecker('student_delete') || permissionChecker('student_view')) { ?>
                                                        <td data-title="<?=$this->lang->line('action')?>">
                                                            <?php
                                                                echo btn_view('student/view/'.$student->srstudentID."/".$set."/".$setcampusID, $this->lang->line('view'));
                                                                if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
                                                                    echo btn_edit('student/edit/'.$student->srstudentID."/".$set."/".$setcampusID, $this->lang->line('edit'));
                                                                    echo btn_delete('student/delete/'.$student->srstudentID."/".$set."/".$setcampusID, $this->lang->line('delete'));
                                                                }
                                                            ?>
                                                        </td>
                                                    <?php } ?>
                                    ...
                                    <div id="tab<?=$section->classesID.$section->sectionID?>" class="tab-pane">
                                        <div id="hide-table">
                                            <table class="table table-striped table-bordered table-hover dataTable no-footer">
                                                ...
                                                <tbody>
                                                    <?php if(!empty($allsection[$section->sectionID])) { $i = 1; foreach($allsection[$section->sectionID] as $student) { if($section->sectionID === $student->srsectionID) { ?>
                                                        <tr>
                                                            <td data-title="<?=$this->lang->line('slno')?>">
                                                                <?php echo $i; ?>
                                                            </td>
...
var setcampusID = "<?php echo $setcampusID; ?>";
if(setcampusID==''){
    getInfoCampusView(0);
}else{
    getInfoCampusView(setcampusID);
}

function getInfoCampusView(setcampusID)
{ 
    $.ajax({
        async: false,
        type: 'POST',
