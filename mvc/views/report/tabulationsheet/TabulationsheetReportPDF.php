            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <?php if(!empty($mandatorysubjects)) { foreach ($mandatorysubjects as $mandatorysubject) { ?>
                        ...
                        <?php if(!empty($optionalsubjects)) { ?>
                            <th colspan="<?=(count($markpercentages) +1) ?>">
                                <?php $i = 1; if(!empty($optionalsubjects)) { foreach ($optionalsubjects as $optionalsubject) { ?>
                    ...
                    <tr>
                        <?php if(!empty($mandatorysubjects)) { foreach ($mandatorysubjects as $mandatorysubject) { ?>
                            <?php if(!empty($markpercentages)) { foreach ($markpercentages as $markpercentageKey => $markpercentage) { ?>
                                <th><?=$markpercentage->markpercentagetype[0]?></th>
                            <?php } } ?>
                            <th><?=$this->lang->line('tabulationsheetreport_total')?></th>
                        <?php } } ?>

                        <?php if(!empty($optionalsubjects)) { foreach ($optionalsubjects as $optionalsubject) { ?>
                            <?php if(!empty($markpercentages)) { foreach ($markpercentages as $markpercentageKey => $markpercentage) { ?>
                                <th><?=$markpercentage->markpercentagetype[0]?></th>
                            <?php } } ?>
                        <?php break; } ?>
