            ...
            <div class="col-sm-12">
                <?php if((($siteinfos->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) || ($this->session->userdata('usertypeID') != 3)) { ?>
                    <h5 class="page-header">
                        <?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1) || permissionChecker('lecture_add')) { ?>
                            
                                <a href="<?php echo base_url('lecture/add') ?>">
                ...
                <div id="hide-table">
                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer">
                        ...
                        <tbody>
                            <?php if(!empty($syllabuss)) {$i = 1; foreach($syllabuss as $lecture) { ?>
                                <tr>
                                    ...
                                    <td data-title="<?=$this->lang->line('syllabus_uploder')?>">
                                        <?php echo getNameByUsertypeIDAndUserID($lecture->usertypeID, $lecture->userID); ?>
                                    </td>
                                    <td data-title="<?=$this->lang->line('syllabus_file')?>">
                                        <?php echo namesorting($lecture->originalfile, 20); ?>
                                    </td>
                                    <td data-title="<?=$this->lang->line('action')?>">
                                        <?php echo btn_download('lecture/download/'.$lecture->lectureID, $this->lang->line('download')) ?>
                                          <?php if(permissionChecker('lecture_edit') || permissionChecker('lecture_delete')) { ?>
                                            <?php echo btn_edit('lecture/edit/'.$lecture->lectureID.'/'.$set, $this->lang->line('edit')) ?>
                                            <?php echo btn_delete('lecture/delete/'.$lecture->lectureID.'/'.$set, $this->lang->line('delete')) ?>
                                        <?php } ?>
                                    </td>
                                </tr>
                            <?php $i++; }} ?>
    ...
    $('#classesID').change(function() {
        ...
        if(classesID == 0) {
            ...
            $.ajax({
                ...
                data: "id=" + classesID + "&campusID=" + campusID,
                dataType: "html",
...
if(campusID==''){
    ...
    getInfoCampusView(campusID);
}

function getInfoCampusView(campusID)
{ 
    $.ajax({
        async: false,
        type: 'POST',
        ...
        success: function(data) {
            $('#classesID').html(data);
            $('#classesID option[value="<?=$set?>"]').attr('selected','selected');
        }
    }); 
}        
</script>
