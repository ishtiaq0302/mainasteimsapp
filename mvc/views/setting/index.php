    ...
    <form class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
        <div class="box-body">
            <fieldset class="setting-fieldset">
                ...
                <div class="col-sm-4">
                        <div class="form-group <?=form_error('campusID') ? 'has-error' : ''?>">
                            <div class="col-sm-12">
                                <label for="campusID"><?=$this->lang->line("campus_name")?>&nbsp;<i class="fa fa-question-circle" data-toggle="tooltip" data-placement="top" title="a graduate or former student class of a specific school"></i>
                                </label>
                                    <?php
                                        if(!empty($getCampus)) {
                                            foreach($getCampus as $campus) {
                                                $campusArrays[$campus['campusID']] = $campus['name'];
                                            }
                                        }
                                        echo form_dropdown("campusID", $campusArrays, set_value("campusID", $setcampusID), "id='campusID' class='form-control select2'");
                                    ?>
            ...
            <fieldset class="setting-fieldset">
                ...
                <div class="row">
                    <div class="form-group">
                        <div class="col-sm-12">
                            <?php
                                if(!empty($markpercentages)) {
                                    foreach ($markpercentages as $key => $markpercentage) {
                                        $checkbox = '';
                                        $compress = 'mark_'.str_replace(' ', '', $markpercentage->markpercentageID);
                                        $markarray = unserialize($settingarray->mark);

                                        if(isset($markarray[$compress])) {
                                            if($markarray[$compress] == 1) {
                                                $checkbox = 'checked';
                                            } else {
                                                $checkbox = ''; 
                                            }
                    ...
                    <div class="form-group <?php if(form_error('attendance_notification_template')) { echo 'has-error'; } ?>" id="attendance_notification_template_div">
                        <div class="col-sm-12">
                            ...
                            <?php
                                ...
                                if(!empty($attendance_notification_templates)) {
            ...
            <fieldset class="setting-fieldset">
                <legend class="setting-legend">Zoom</legend>
                <div class="col-sm-12">
                    <div class="form-group <?php if(form_error('zoom')) { echo 'has-error'; } ?>">
                        <div class="col-sm-12">
                            <div class="form-group <?=form_error('zoom_api_key') ? 'has-error' : ''?>">
                                <div class="col-sm-6">
                                    <label for="zoom_api_key">Client ID&nbsp;<i class="fa fa-question-circle" data-toggle="tooltip" data-placement="right" title="Set site zoom_api_key code"></i>
                                    </label>
                                    <input type="text" class="form-control" id="zoom_api_key" name="zoom_api_key" value="<?=set_value('zoom_api_key', $setting->zoom_api_key)?>" >
                                    <span class="control-label">
                                        <?php echo form_error('zoom_api_key'); ?>
                                    </span>
                                </div>

                                <div class="col-sm-6">
                                    <label for="zoom_api_secret">Client Secret&nbsp;<i class="fa fa-question-circle" data-toggle="tooltip" data-placement="right" title="Set site zoom_api_secret code"></i>
                                    </label>
                                    <input type="text" class="form-control" id="zoom_api_secret" name="zoom_api_secret" value="<?=set_value('zoom_api_secret', $setting->zoom_api_secret)?>" >
                                    <span class="control-label">
                                        <?php echo form_error('zoom_api_secret'); ?>
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <a id="<?=$theme->themesID?>" data-toggle="tooltip" data-placement="top" title="" data-original-title="<?=$theme->themename?>"
                         ...
                         data-skin="skin-green-light" style="display: block; box-shadow: 0 0 3px rgba(0,0,0,0.4); cursor: pointer;" class="clearfix full-opacity-hover backendThemeEvent">
                            ...
                            <div>
                                ...
                                <?php  ?>
                                        <?php if($unique_setting->backend_theme == strtolower(str_replace(' ', '', $theme->themename)))  {?>
...
$('#campusID').change(function(event) {
    ...
    window.location.href = "<?=base_url('setting/index/')?>"+campusID;
});
