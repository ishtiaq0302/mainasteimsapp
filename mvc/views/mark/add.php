    ...
    <div class="box-body">
        <div class="row">
            <div class="col-sm-12">
                                ...
                                <div class="col-md-3">
                                    <div class="<?php echo form_error('studentID') ? 'form-group has-error' : 'form-group'; ?>" >
                                        <label for="studentID" class="control-label">
                                            <?=$this->lang->line('mark_student')?> <span class="text-red">*</span>
                                        </label>
                                        <?php
                                            ...
                                            echo form_dropdown("studentID", $array, set_value("studentID", $set_student), "id='studentID' class='form-control select2'");
                                        ?>
                                    </div>
                                </div>
                            </div>
                ...
                <?php if(!empty($sendExam) && !empty($sendClasses) && !empty($sendSection) && !empty($sendSubject)) { ?>
            ...
            <div class="col-sm-12">
                ...
                <div id="hide-table">
                    <table class="table table-striped table-bordered table-hover dataTable no-footer">
                        <thead>
                            <tr>
                                <th><?=$this->lang->line('slno')?></th>
                                <th><?=$this->lang->line('mark_subject')?> Code</th>
                                <th><?=$this->lang->line('mark_subject')?></th>
                                <th><?=$this->lang->line('mark_type')?></th>
                                <?php
                                    foreach ($markpercentages as $data) {
                                        echo "<th>$data->markpercentagetype ($data->percentage)</th>";
                                    }
                        ...
                        <tbody>
                            <?php if(!empty($subjects)) {
                                ...
                                foreach($subjects as $subject) {
                                    foreach ($marks as $mark) { 
                                        if($subject->subjectID == $mark->subjectID) {   ?>
                                <tr>
                                    <td data-title="<?=$this->lang->line('slno')?>">
                                        <?php echo $i; ?>
                                    </td>
                                    <td data-title="<?=$this->lang->line('mark_subject')?> code">
                                        <?php echo $subject->subject_code; ?>
                                        <?php /* echo profileproimage($student->photo); */ ?>
                                    </td>
                                    <td data-title="<?=$this->lang->line('mark_subject')?>">
                                        <?php echo $subject->subject; ?>
                                    </td>
                                    <td data-title="<?=$this->lang->line('mark_type')?>">
                                        <?php echo $subject->type; ?>
                                    </td>
                                    <?php
                                        foreach ($markpercentages as $data) {
                                            echo "<td data-title='$data->markpercentagetype'>";
                                                echo "<input class='form-control mark' type='number' name='mark-".$markwr[$subject->subjectID][$data->markpercentageID]."'value='".$markRelations[$subject->subjectID][$data->markpercentageID]."' min='0' max='".$data->percentage."' />";
                                            echo "</td>";
                                        }
                                    ?>
                ...
                <input type="button" class="btn btn-success col-sm-2 col-xs-12" id="add_mark_student" name="add_mark" value="<?=$this->lang->line("add_sub_mark")?>" />
                ...
                <script type="text/javascript">
                    ...
                    $("#add_mark_student").click(function() {
                        var inputs = "";
                        var inputs_value = "";
                        var mark = $('input[name^=mark]').map(function(){
                            return { mark: this.name , value: this.value, };
                        ...
                        $.ajax({
                            type: 'POST',
                            url: "<?=base_url('mark/mark_send_student')?>",
                            data: {"examID" : "<?=$set_exam?>", "classesID" : "<?=$set_classes?>", "studentID" : "<?=$set_student?>", "inputs" : mark},
                            dataType: "html",
                            success: function(data) {
                                var response = jQuery.parseJSON(data);
                                if(response.status) {
...
{ 
    $.ajax({
        ...
        success: function(data) {
            $('#classesID').html(data);
            $('#classesID option[value="'+<?=$set_classes?>+'"]').attr('selected','selected');
        }
    }); 
    ...
    $.ajax({
        async: false,
        type: 'POST',
        url: "<?=base_url('universalfunction/examcall')?>",
        data: "id=" + campusID,
        dataType: "html",
        success: function(data) {
            $('#examID').html(data);
            $('#examID option[value="'+<?=$set_exam?>+'"]').attr('selected','selected');
        }
    });
} 
</script>
